...
public:
    static bool usesIndependentNeighborList(const Integrator& integrator) {
        if (dynamic_cast<const DPDIntegrator*>(&integrator) != nullptr)
            return true;
        if (const auto* compound = dynamic_cast<const CompoundIntegrator*>(&integrator)) {
            for (int i = 0; i < compound->getNumIntegrators(); i++)
                if (usesIndependentNeighborList(compound->getIntegrator(i)))
                    return true;
        }
        return false;
    }
