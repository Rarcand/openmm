...
void CommonUpdateStateDataKernel::loadCheckpoint(ContextImpl& context, istream& stream) {
    ...
    // A preserved-state reinitialization can change the selected ordering mode.
    bool normalized = false;
    if (cc.getNonbondedUtilities().getUsesStableAtomOrder()) {
        for (int i = 0; i < cc.getNumAtoms(); i++) {
            if (cc.getAtomIndex()[i] != i) {
...
void CommonCalcCustomCentroidBondForceKernel::initialize(const System& system, const CustomCentroidBondForce& force) {
    ...
    for (int i = 0; i < numBonds; i++) {
        ...
        if (cc.getNonbondedUtilities().getUsesStableAtomOrder()) {
            vector<int> atoms;
            for (int group : groups)
                atoms.insert(atoms.end(), groupParticleVec.begin()+groupOffsetVec[group],
                             groupParticleVec.begin()+groupOffsetVec[group+1]);
            cc.registerRecenterGroup(atoms);
        }
