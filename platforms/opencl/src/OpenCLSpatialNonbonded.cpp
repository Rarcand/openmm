...
void OpenCLSpatialNonbonded::initialize(const System& system) {
    ...
    identityOrdering = !nb.useNeighborList;
    packedExclusions = nb.useNeighborList && nb.canOmitExcludedPairs;
    for (int i = 0; i < system.getNumForces(); i++)
        if (dynamic_cast<const CustomNonbondedForce*>(&system.getForce(i)))
    ...
    if (packedExclusions) {
        ...
        if (columns.size() > (numeric_limits<int>::max()-2*(size_t) blocks)/4)
            throw OpenMMException("Too many spatial exclusions for the block table");
        ...
        blockCounts.initialize<unsigned int>(cc, blocks, "spatialBlockCounts");
        blockRows.initialize<mm_int2>(cc, blocks, "spatialBlockRows");
        blockTable.initialize<mm_int2>(cc, 4*(size_t) numRecords+2*blocks, "spatialBlockTable");
        blockFilters.initialize<unsigned int>(cc, 8*blocks, "spatialBlockFilters");
        originalRows.upload(rows);
        originalColumns.upload(columns);
        neighborMasks.initialize<unsigned int>(cc, nb.interactingAtoms.getSize(), "spatialNeighborMasks");
        ...
        nb.maxExclusions = 1;
        vector<unsigned int> blockOrder(blocks);
        for (int i = 0; i < blocks; i++) blockOrder[i] = i;
        nb.sortedBlocks.upload(blockOrder);
        ...
        if (!identityOrdering) nb.exclusionTiles.resize(numRecords);
    ...
    if (packedExclusions) source += OpenCLKernelSources::spatialPackedExclusions;
    ...
    if (packedExclusions)
        for (const string& name : {"spatialMapExclusions", "spatialDiagonalMasks", "blockCountExclusions", "blockLayout", "blockInsertExclusions"})
            kernels[name] = program->createKernel(name);
...
void OpenCLSpatialNonbonded::reorder() {
    if (identityOrdering) {
        vector<int> identity(cc.getPaddedNumAtoms());
        for (int i = 0; i < identity.size(); i++) identity[i] = i;
        order.upload(identity);
        inverseOrder.upload(identity);
        remapExclusions();
...
void OpenCLSpatialNonbonded::remapExclusions() {
    if (packedExclusions) {
        execute("spatialMapExclusions", {&originalColumns, &inverseOrder, &mappedColumns}, numRecords);
        execute("spatialDiagonalMasks", {&order, &originalRows, &mappedColumns, &nb.exclusions}, cc.getPaddedNumAtoms());
        cc.clearBuffer(blockCounts);
        cc.clearBuffer(blockTable);
        cc.clearBuffer(blockFilters);
        execute("blockCountExclusions", {&order, &originalRows, &mappedColumns, &blockCounts}, cc.getNumAtoms());
        execute("blockLayout", {&blockCounts, &blockRows}, 1);
        execute("blockInsertExclusions", {&order, &originalRows, &mappedColumns, &blockRows, &blockTable, &blockFilters}, cc.getNumAtoms());
...
bool OpenCLSpatialNonbonded::usesBoundsGather() const {
    return (inverseMerge || packedExclusions) && nb.useNeighborList && nb.numTiles > 0;
}
void OpenCLSpatialNonbonded::prepare() {
    forcesPending = false;
    if (nb.useNeighborList) {
        Vec3 box[3];
    ...
    if (reorderCount == 0 || (!identityOrdering && cc.getStepsSinceReorder() >= reorderInterval)) reorder();
    if (identityOrdering) {
        if (cc.getStepsSinceReorder() >= reorderInterval) cc.setStepsSinceReorder(0);
        return;
    }
...
bool OpenCLSpatialNonbonded::mergeForces() {
    if (!forcesPending || identityOrdering) return false;
    if (inverseMerge) {
...
void OpenCLSpatialNonbonded::resizeNeighborMasks() {
    if (packedExclusions) neighborMasks.resize(nb.interactingAtoms.getSize());
}

OpenCLArray& OpenCLSpatialNonbonded::getPositions() {
    return identityOrdering ? cc.unwrap(cc.getPosq()) : positions;
}

OpenCLArray& OpenCLSpatialNonbonded::getForces() {
    return identityOrdering ? cc.unwrap(cc.getLongForceBuffer()) : forces;
}
