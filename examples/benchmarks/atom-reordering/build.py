...
def main():
    ...
    a=p.parse_args()
    command=['cmake','-S',str(a.source.resolve()),'-B',str(a.build_dir.resolve()),'-G',a.generator,
             '-DCMAKE_BUILD_TYPE=Release','-DBUILD_TESTING='+('ON' if a.tests else 'OFF')]
    for backend in ('CUDA','OpenCL'):
        command.append('-DOPENMM_BUILD_'+backend.upper()+'_LIB='+('ON' if backend in a.backends else 'OFF'))
    for feature in ('HIP_LIB','CPU_LIB','PYTHON_WRAPPERS','C_AND_FORTRAN_WRAPPERS','AMOEBA_PLUGIN','DRUDE_PLUGIN','RPMD_PLUGIN','PME_PLUGIN','EXAMPLES'):
        command.append('-DOPENMM_BUILD_'+feature+'=OFF')
    if os.name=='nt' and 'CUDA_PATH' in os.environ:
        cuda=Path(os.environ['CUDA_PATH'])
        command+=['-DCUDAToolkit_ROOT='+str(cuda),'-DNVRTC_LIB='+str(cuda/'lib/x64/nvrtc.lib'),
                  '-DOPENCL_INCLUDE_DIR='+str(cuda/'include'),'-DOPENCL_LIBRARY='+str(cuda/'lib/x64/OpenCL.lib')]
    ...
    subprocess.run(['cmake','--build',str(a.build_dir.resolve()),'--config','Release','--parallel',str(a.jobs),'--target']+targets,check=True)
