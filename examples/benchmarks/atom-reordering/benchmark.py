...
from one process as independent sessions. See PROTOTYPE.md and MEASUREMENTS.md.
"""
import argparse
import ctypes
import hashlib
...
def properties(backend, precision, mode):
    ...
    if backend == 'OpenCL': props['OpenCLPlatformIndex'] = '0'
    # Omit experimental properties when measuring pristine OpenMM.
    if mode != 'pristine':
...
def check_snapshot(actual, expected):
    ...
    assert np.sqrt(np.mean(delta**2)) <= 1e-3+1e-4*scale
    assert np.max(np.abs(delta)) <= 1e-3+1e-3*scale
    np.testing.assert_allclose(actual['energy'], expected['energy'], rtol=1e-4, atol=1e-3)
...
def main():
    ...
    p.add_argument('--timestep', type=float, default=.002, help='ps')
    a = p.parse_args()
    ...
    result = dict(backend=a.backend, precision=a.precision, requested_mode=a.mode,
                  properties={k: platform.getPropertyValue(context,k) for k in platform.getPropertyNames()},
                  libraries=libraries, system_sha256=digest(a.system), state_sha256=digest(a.state),
                  steps_per_batch=a.steps, warmup=a.warmup, seconds=samples,
